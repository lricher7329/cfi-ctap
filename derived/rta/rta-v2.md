...
The University of Alberta possesses a unique convergence of capabilities: Amii with 10+ CIFAR AI Chairs; Connect Care spanning 682 sites and 4.4 million Albertans; over $400M in discovery infrastructure (PRAIRIE Hub, ACTM, Li Ka Shing Institute of Virology, SPP-ARC and the Cryo-EM Facility, Glycomics Institute of Alberta, TMIC); and proven AI deployment (Dr. Mitchell's AI Scribe across 10 Alberta EDs). No other Canadian institution combines AI leadership, unified health system integration, advanced biomanufacturing, and translational infrastructure in a single ecosystem.
...
## 2.2 The Translation Challenge: Why Discovery ≠ Patient Benefit

**A promising discovery in the laboratory is only the beginning of a 10-15 year journey to reach patients.** University reviewers familiar with basic science may not fully appreciate the extraordinary complexity—and the multiple points of failure—involved in translating a pre-clinical finding into an approved therapy. CTAP addresses this "valley of death" by providing integrated infrastructure across every stage.

### The Current Reality: A Fragmented Journey

Consider a University of Alberta researcher who discovers a promising therapeutic compound. Today, that researcher faces:

| Stage | Current Timeline | Key Challenges | Failure Points |
|-------|------------------|----------------|----------------|
| **1. Pre-clinical validation** | 2-4 years | Access to GLP facilities, toxicology, formulation | 95% of compounds fail |
| **2. Regulatory preparation** | 6-12 months | IND/CTA preparation, CMC documentation | Expertise gap |
| **3. Ethics & institutional approval** | 6-18 months | Multiple REB submissions, contracts, budgets | Administrative burden |
| **4. Trial activation** | 9-12 months | Site selection, staff training, system setup | Resource constraints |
| **5. Patient recruitment** | 1-3 years | Finding eligible patients, consent, enrollment | 80% of trials miss targets |
| **6. Data collection & monitoring** | 2-5 years | Case report forms, safety monitoring, site visits | Quality issues |
| **7. Analysis & reporting** | 6-12 months | Statistical analysis, regulatory submission | Expertise gap |

**Result:** A 10-15 year timeline from discovery to approval, with 90%+ of promising compounds never reaching patients.

### The CTAP-Enabled Future: A Worked Example

**Case Study: The Taurine Supplementation Trial for Long COVID**

This real example from Dr. Gavin Oudit's translational program illustrates CTAP's transformative potential:

**Discovery Phase (Completed):**
Dr. Oudit's laboratory identified that SARS-CoV-2 infection depletes taurine, an amino acid critical for mitochondrial function. Pre-clinical work demonstrated that taurine supplementation restored cellular energy metabolism.

**Current State (Without Full CTAP):**
- Protocol development: 4 months (manual literature review, iterative drafting)
- Ethics approval: 6 months (standard REB timelines)
- Site activation: 3 months (contracts, training, system setup)
- Recruitment: Ongoing challenge (manual chart review for eligibility)
- Data management: Excel spreadsheets, manual entry
- **Total time to first patient:** 13 months
- **Current enrollment:** ~50 patients/year

**CTAP-Enabled State (What Infrastructure Investment Achieves):**

| Component | CTAP Infrastructure | Impact |
|-----------|---------------------|--------|
| **Protocol Design** | CRAIDL Protocol Agent synthesizes literature, simulates feasibility, generates compliant draft | 4 months → 2 weeks |
| **Ethics Preparation** | CRAIDL Ethics Agent pre-populates REB forms from protocol, flags regulatory requirements | 6 months → 6 weeks (streamlined submission) |
| **Patient Identification** | Connect Care/AHS Snowflake queries identify eligible patients across 4.4M Albertans | Manual review → Real-time cohort identification |
| **Biospecimen Collection** | Living Biorepository with FHIR-linked consent, automated outcome annotation | Static samples → Dynamic discovery engine |
| **Data Capture** | AI Scribe integration, structured data from clinical encounters | Manual entry → Voice-first automated capture |
| **Safety Monitoring** | TRE-enabled real-time adverse event detection across provincial EMR | Periodic review → Continuous surveillance |
| **Analysis** | OMOP-standardized data, CRAIDL analysis agent | Months → Weeks |

**Projected CTAP-enabled timeline:** First patient in 45 days (Alberta FAST target), enrollment capacity 500+ patients/year through decentralized participation.

### Why Each Infrastructure Component Matters

**Without the Living Biorepository (Suite 2):** Blood samples sit frozen without connection to patient outcomes. When results are published 3 years later, researchers cannot link molecular findings to clinical trajectories.

**Without the TRE (Suite 4):** Each new research question requires a 6-12 month data access request. Researchers cannot query provincial data to validate findings or identify participants.

**Without CRAIDL (Suites 3-4):** Protocol development remains manual—each investigator reinvents the wheel, spending months on tasks that could be completed in days.

**Without Translational Integration (Suite 3):** Core facilities operate in silos. A trial requiring genomics, proteomics, and imaging must coordinate separately with each facility, with no unified data return pipeline.

**Without Trial Innovation Tools (Suite 5):** Rural and Indigenous participants cannot participate without travelling to Edmonton. Digital health devices generate data that cannot be integrated with trial systems.

## 2.3 Why the University of Alberta, Why Now

**Patients cannot wait.** The University of Alberta has made transformative investments in compute, AI expertise, and biomanufacturing. What is missing is **the glue**: an AI-native platform connecting these assets into a seamless pathway from discovery to patient benefit.
...
CTAP is integrated with the U of Alberta Clinical Trials (UACT) consortium and led through the **Kipnes Health Research Institute**. The platform operates through five tightly coupled domains unified by a three-zone Trusted Research Environment (TRE):

**Domain 1: AI-Enabled Operations Hub.** CRAIDL (Clinical Research Assistants for Intelligent Design and anaLysis)—8 AI agents developed with Amii fellow—simulate protocol feasibility, predict recruitment bottlenecks, and automate regulatory documentation. The work will build on four agents already validated at the investigator scale; CFI investment enables enterprise deployment. Target: 40–50% reduction in trial startup time.
...
**Domain 3: Trusted Research Environment.** Three-zone TRE building on an environment already deployed at the U of Alberta in partnership with AWS (~$1.35M prior institutional investment including TRE architecture, AWS credits supporting AI Scribe across 10 Alberta EDs, and AWS ProServe consulting). Zone 1 (identifiable, AHS-controlled) leverages the AHS Snowflake research instance for Connect Care bulk extracts; Zone 2 (privacy by design de-identification, AI processing, data extraction layer); Zone 3 (de-identified research data, OMOP analytics). The CFI request extends this proven foundation to a prospective "Trials Zone" meeting Health Canada Division 5 and ICH E6(R3) requirements—a 2.4:1 leverage ratio on prior investment.

**Domain 4: Clinical Trials - Translational Integration.** Trial-to-facility interface layer connecting trials with UAlberta core facilities such the Advanced Cell Exploration (ACE) Core, SPP-ARC and the Cryo-EM facility, GIA, TMIC, Alberta Proteomics) through protocol integration services, quality bridge infrastructure, and data return pipelines.

## 2.5 CRAIDL: The AI-First Clinical Trials Engine

CRAIDL (Clinical Research Assistants for Intelligent Design and Analysis), envisioned by Justin Ezekowicz, leverages agentic AI—agents that function autonomously under human oversight—developed in collaboration with Amii and CIFAR AI Chairs (Mohamed Abdalla, Ross Mitchell). Built on the Canadian VIGOUR Centre's international leadership in cardiovascular trials.

**The In Silico Assessment Agent**—Canada's first prospective trial intelligence capability—an AI-enabled modelling-and-simulation capability that continuously evaluates a candidate intervention (drug, biologic, device) and a proposed protocol by running *virtual experiments* —using mechanistic models (e.g., PBPK/QSP), statistical trial simulation, and (where appropriate) real-world/registry priors—to reduce uncertainty *before* spending time and money on patients. Simulates trial outcomes before enrollment to address the 40-60% trial failure rate. Builds on the work of the Real World Evidence Unit (Scott Klarenbach, Lawrence Richer) and MS Health Research Data Platform (funded by PrairiesCan and University Hospital Foundation).

**Human-in-the-Loop:** All outputs require human review—protocol drafts by PI/statistician, safety flags by medical monitor, regulatory packages by regulatory affairs. The In Silico Assessment Agent is designed to *amplify* expert capacity rather than replace it, shifting human effort from manual analysis to higher-value judgment. The agent rapidly synthesises evidence, runs scenario analyses, and pre-structures outputs so that PIs and statisticians can focus on scientific intent and design trade-offs, medical monitors on clinically meaningful safety signals, and regulatory affairs on strategic positioning and defensibility. Human review remains mandatory, but is faster, more focused, and more consistent—allowing a small number of highly trained experts to oversee more studies, with greater rigour and traceability, than would be feasible without AI-enabled support.
...
## 2.7 Flagship Programs

CTAP delivers impact through five flagship programs that operationalise its end-to-end clinical trials acceleration infrastructure:
...
## 2.8 Feasibility and International Context

**Operational Foundation:** CTAP is built on a strong, already-operational foundation. This includes a coordinated clinical trials infrastructure through UACT/CTO; a mature national trials organisation in the Canadian VIGOUR Centre, with more than 50 validated trial templates available to support CRAIDL agent training; and fully functioning facilities within the Canadian BioSample Repository and Advanced Cell Therapy Manufacturing (ACTM), supported by validated GMP workflows and SOPs. AI capability is anchored by internationally recognised Amii/CIFAR AI Chairs, with production-scale deployment of AI scribe technologies and four CRAIDL agents already operating at the investigator level. Together, these assets enable a phased 4–5 year implementation plan, with demonstrable early wins achievable within the first 18–24 months.

**International Context:** Globally, major initiatives such as the UK Biobank, All of Us, and Germany's National Decade Against Cancer have developed individual components of modern clinical research infrastructure. However, no existing platform integrates, within a single operational environment: (1) GMP-grade manufacturing, (2) structural and systems biology, (3) metabolomics and glycomics at scale, (4) a top-tier artificial intelligence institute, (5) a mature, high-throughput clinical trials organisation, and (6) province-wide electronic medical record integration. By unifying these capabilities, **CTAP represents Canada's first AI-native clinical trials acceleration platform**, purpose-built to translate discovery into first-in-human and population-scale trials with unprecedented speed, rigour, and scalability.

# 3. Research Team

**Leadership:** Lawrence Richer - Associate Dean Research, College of Health Sciences and Centre Director of the Northern Alberta Clinical Trials and Research Center, lead of Data Coordinating Center for RareKids-CAN; Jason Acker - Associate Vice-President Research Integrity Support with expertise in cryo-preservation

**Clinical Trials:** Justin Ezekowicz and Canadian VIGOUR Centre (50+ trials), Peter Senior and the Alberta Diabetes Institute, Gavin Oudit (translational science program in Long COVID), Neesh Pannu (pragmatic, EMR-embedded clinical trials), Michael Houghton (Nobel 2020, vaccine challenge trials).
...
CTAP aligns with the University of Alberta's **Access, Community, and Belonging (ACB)** framework and operationalizes CIHR's Strategy for Patient-Oriented Research (SPOR) with patient partners as co-investigators with governance authority. Canada's first **Indigenous Clinical Trials Unit (ICTU)**, led by Wayne Clark, ensures OCAP principles govern Indigenous data through TRE-enforced access controls, with culturally safe clinical spaces (smudging, Elder consultations, Indigenous language signage) and decentralized tools extending participation to remote communities.
...
**Indigenous Digital Health (ICTU):** Wayne Clark and Tibetha Kemble's Indigenous Clinical Trials Unit leads the testing of digital health interventions tailored for Indigenous communities. Suite 5 telehealth infrastructure ($100,000 for 20 rural/Indigenous sites) enables trial participation from remote First Nations, Métis settlements, and northern communities without requiring travel to Edmonton. AI Scribe's local data processing helps to address Indigenous data sovereignty requirements under OCAP principles. The eConsent platform supports Indigenous language options and culturally appropriate consent processes developed in partnership with community Elders and knowledge keepers.
